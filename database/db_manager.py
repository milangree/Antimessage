...
class DatabaseManager:
    ...
    async def migrate_database(self, db):
        ...
        try:
            await db.execute('ALTER TABLE users ADD COLUMN verification_mode TEXT DEFAULT NULL')
            logging.info("数据库迁移：成功为 'users' 表添加 'verification_mode' 列。")
        except aiosqlite.OperationalError as e:
            if "duplicate column name" not in str(e):
                raise e

        try:
            await db.execute('ALTER TABLE users ADD COLUMN verification_image_type TEXT DEFAULT NULL')
            logging.info("数据库迁移：成功为 'users' 表添加 'verification_image_type' 列。")
        except aiosqlite.OperationalError as e:
            if "duplicate column name" not in str(e):
                raise e
